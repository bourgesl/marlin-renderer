...
final class DirtyIntArrayCache implements MarlinConst {

    private WeakReference<Bucket[]> refBuckets = null;
    private final int bucketCapacity;
    final CacheStats stats;

    DirtyIntArrayCache(final int bucketCapacity) {
        this.bucketCapacity = bucketCapacity;
        this.stats = (DO_STATS) ? new CacheStats("DirtyIntArrayCache") : null;
    }
    ...
    static final class Reference {
        ...
        final int[] initial;
        private final DirtyIntArrayCache cache;

        Reference(final DirtyIntArrayCache cache,
        ...
        int[] getArray(final int length) {
            ...
            if (DO_LOG_OVERSIZE) {
                logInfo("DirtyIntArrayCache: getArray[oversize]: length=\t"
                        + length);
            }
                          ...
                          final int needSize)
        {
            ...
            if (DO_LOG_WIDEN_ARRAY) {
                logInfo("DirtyIntArrayCache: widenArray[" + res.length
    ...
    static final class Bucket {
        ...
        void putArray(final int[] array, final int length)
        {
            if (length != arraySize) {
                if (DO_CHECKS) {
                    MarlinUtils.logInfo("DirtyIntArrayCache: bad length = "
                                        + length);
                }
                return;
            }
            ...
            if (arrays.length > tail) {
                ...
                MarlinUtils.logInfo("DirtyIntArrayCache: array capacity exceeded !");
            }
        }
    }
