...
GC=true
FORK=1 # 0: no-fork
ITER=5 # use 1 for perfasm or stack
...
#JAVA_OPTS="-Xms1g -Xmx1g -XX:+UseParallelGC -XX:-BackgroundCompilation -XX:+AlwaysPreTouch -XX:+UseTransparentHugePages"
JAVA_OPTS="-Xms1g -Xmx1g -XX:+UseParallelGC -XX:NewRatio=2 -XX:-BackgroundCompilation -XX:+AlwaysPreTouch -XX:+UseTransparentHugePages"  # gc profiler
...
java $JAVA_OPTS -jar $DIR/target/marlin-test-bench.jar -gc $GC -t 1 -f $FORK -i $ITER -prof $PROF $OPTS 1> "prof-$PROF-area-$LEN.log" 2> "prof-$PROF-area-$LEN.err" &

PID=$!
echo "JAVA PID: $PID"
sudo cset shield --shield --threads --pid $PID

tail -f "prof-$PROF-area-$LEN.log"
