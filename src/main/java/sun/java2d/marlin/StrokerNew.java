...
// LBO: try double precision like Stroker.Double()

// TODO: some of the arithmetic here is too verbose and prone to hard to
// debug typos. We should consider making a small Point/Vector class that
// has methods like plus(Point), minus(Point), dot(Point), cross(Point)and such
final class StrokerNew implements PathConsumer2D, MarlinConst {

    private static final int MOVE_TO = 0;
    private static final int CLOSE = 1;
    private static final int DRAWING_OP_LINETO = 2;
    private static final int DRAWING_OP_QUADTO = 3;
    private static final int DRAWING_OP_CURVETO = 4;
     ...
     */
    StrokerNew(final RendererContext rdrCtx) {
        this.rdrCtx = rdrCtx;
     ...
     */
    StrokerNew init(PathConsumer2D pc2d,
              float lineWidth,
              int capStyle,
              int joinStyle,
              float miterLimit)
    ...
    private boolean drawRoundJoin(float x, float y,
    ...
    private boolean drawMiter(final float pdx, final float pdy,
    ...
    public void lineTo(float x1, float y1) {
        ...
        drawJoin(cdx, cdy, cx0, cy0, dx, dy, cmx, cmy, mx, my, false);

        // TODO: use stack to emit / cancel last segment ?
    ...
    public void closePath() {
        ...
        drawJoin(cdx, cdy, cx0, cy0, sdx, sdy, cmx, cmy, smx, smy, false);
                          ...
                          boolean curve)
    {
        if (prev < DRAWING_OP_LINETO) {
            ...
            if (joinStyle == JOIN_MITER) {
                done = drawMiter(pdx, pdy, x0, y0, dx, dy, omx, omy, mx, my, cw);
                // TODO: check tan(angle) smaller than length * half width ?
//                drawMiter(pdx, pdy, x0, y0, dx, dy, omx, omy, mx, my, !cw);
            } else if (joinStyle == JOIN_ROUND) {
//                drawMiter(pdx, pdy, x0, y0, dx, dy, omx, omy, mx, my, !cw);
                done = drawRoundJoin(x0, y0,
                              omx, omy,
                              mx, my, cw,
                              ROUND_JOIN_THRESHOLD);
            }
            // curve only bug ?
            if (!done || curve || this.prev > DRAWING_OP_LINETO || joinStyle == JOIN_BEVEL) {
                // LBO: avoid 1 intermediate point ?
//                drawMiter(pdx, pdy, x0, y0, dx, dy, omx, omy, mx, my, !cw);
                emitLineTo(x0, y0, !cw);
            }
        }
        prev = DRAWING_OP_LINETO; // ambiguous
    }
...
****************************** END WORKAROUND *******************************/
                                  ...
                                  float x3, float y3)
    {
        ...
        final float mx = offset[0][0];
        final float my = offset[0][1];
        drawJoin(cdx, cdy, cx0, cy0, dxs, dys, cmx, cmy, mx, my, true);
        ...
        this.cmx = (lp[kind - 2] - rp[kind - 2]) / 2f;
        this.cmy = (lp[kind - 1] - rp[kind - 1]) / 2f;
        this.cdx = dxf;
        this.cdy = dyf;
        this.cx0 = xf;
        this.cy0 = yf;
        this.prev = DRAWING_OP_CURVETO;
    }
    ...
    final static class PolyStack {
        private static final byte TYPE_MOVETO  = (byte) 0;
        private static final byte TYPE_LINETO  = (byte) 1;
        private static final byte TYPE_QUADTO  = (byte) 2;
        private static final byte TYPE_CUBICTO = (byte) 3;
        ...
        void pushMove(float x, float y) {
            ensureSpace(2);
            curveTypes[numCurves++] = TYPE_MOVETO;
            // assert(x0 == lastX && y0 == lastY)
            curves[end++] = x;    curves[end++] = y;
        }
        ...
        void pop(PathConsumer2D io) {
            switch(curveTypes[--numCurves]) {
            case TYPE_MOVETO:
                end -= 2;
                io.moveTo(curves[end], curves[end+1]);
                break;
            case TYPE_LINETO:
                end -= 2;
        ...
        public String toString() {
            ...
            while (nc > 0) {
                ...
                e -= l;
                ret += Arrays.toString(Arrays.copyOfRange(curves, e, e + l))
                                       + "\n";
            }
            return ret;
        }
